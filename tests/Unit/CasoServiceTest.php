...
class CasoServiceTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Crear estados de caso necesarios
        EstadoCaso::factory()->create(['estado_id' => 1, 'estado_nombre' => 'Entrevista', 'estado_tipo' => 'pipeline', 'estado_orden' => 1, 'estado_color' => '#3B82F6']);
        EstadoCaso::factory()->create(['estado_id' => 2, 'estado_nombre' => 'En Proceso', 'estado_tipo' => 'pipeline', 'estado_orden' => 2, 'estado_color' => '#F59E0B']);
        EstadoCaso::factory()->create(['estado_id' => 3, 'estado_nombre' => 'Cerrado', 'estado_tipo' => 'cerrado', 'estado_orden' => 3, 'estado_color' => '#10B981']);

        // Crear tipos de trámite
        TipoTramite::factory()->create(['tipo_tramite_id' => 1, 'tramite_nombre' => 'Civil']);
        TipoTramite::factory()->create(['tipo_tramite_id' => 2, 'tramite_nombre' => 'Penal']);
    ...
    {
        // Este test verifica que el método usa lockForUpdate internamente
        // No podemos testear el lock directamente sin mocking, pero verificamos
        // que la transacción se ejecuta y el correlativo es consistente
        $cliente = Cliente::factory()->create();
        $demandado = Demandado::factory()->create();
        $tipoTramite = TipoTramite::first();
        $procurador = Procurador::factory()->create();
    ...
    {
        $caso = Caso::factory()->create(['caso_estado' => 'activo']);
        ...
        $this->assertEquals('2026-01-15', $caso->resolucion_fecha);
    ...
    public function test_close_caso_usa_fecha_actual_si_no_se_proporciona(): void
    {
        ...
        $caso->refresh();
        $this->assertEquals(now()->toDateString(), $caso->resolucion_fecha);
    }
    ...
    public function test_get_index_data_filtra_por_procurador_si_usuario_es_procurador(): void
    {
        ...
        $casoPropio = Caso::factory()->create(['procurador_id' => $procurador->procurador_id]);
        Caso::factory()->create(); // Caso de otro procurador
    ...
    public function test_get_kanban_columns_agrupa_casos_por_estado(): void
    {
        ...
        $estadoEntrevista = EstadoCaso::where('estado_nombre', 'Entrevista')->first();
        $estadoEnProceso = EstadoCaso::where('estado_nombre', 'En Proceso')->first();

        Caso::factory()->count(2)->create(['estado_id' => $estadoEntrevista->estado_id]);
        Caso::factory()->count(1)->create(['estado_id' => $estadoEnProceso->estado_id]);
    ...
    public function test_get_kanban_columns_maneja_casos_sin_cliente_tramite_audiencia(): void
    {
        ...
        $estado = EstadoCaso::where('estado_nombre', 'Entrevista')->first();
        $caso = Caso::factory()->create([
        ...
        $this->assertEquals('Sin cliente', $tarjetas[$caso->caso_numero_expediente][0]);
        $this->assertEquals('Sin trámite', $tarjetas[$caso->caso_numero_expediente][1]);
    }
}