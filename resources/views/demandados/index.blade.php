@extends('layouts.app')
{{--
    Vista: demandados/index
    Propósito: Listado de demandados con buscador en vivo y filtro activo/inactivo. Muestra tarjetas con iniciales, nombre, DNI, teléfono y conteo de casos asociados.
    Variables: $demandados (paginator de modelos Demandado)
    @extends: layouts.app
    @section: content
--}}

@section('title', 'Demandados')
...
<div x-data="{ 
    ...
    <div class="flex flex-col sm:flex-row gap-3 mb-5">
        <div class="relative flex-1">
            ...
            <input type="text" x-model="search" placeholder="Buscar demandados por nombre, DNI o teléfono..." class="w-full rounded-lg pl-9 pr-3 py-2 text-sm outline-none" style="border: 1px solid #E5E7EB; color: #111827; background: #FFFFFF;">
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <a href="{{ route('demandados.show', $demandado->demandado_dni) }}" 
           ...
           onmouseover="this.style.boxShadow='0 4px 12px rgba(0,0,0,0.08)'; this.style.borderColor='#93C5FD';" onmouseout="this.style.boxShadow='none'; this.style.borderColor='#E5E7EB';">
            <div class="flex items-center gap-3 mb-3">
                ...
                <div class="min-w-0">
                    <p class="text-sm font-medium" style="color: #111827;">{{ $demandado->nombre_completo }}</p>
                    <p class="text-xs" style="color: #6B7280;">{{ $demandado->demandado_dni }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3 text-xs" style="color: #6B7280;">
                <span>{{ $demandado->demandado_telefono }}</span>
                <span class="ml-auto">{{ $demandado->casos_count }} {{ $demandado->casos_count === 1 ? 'caso' : 'casos' }}</span>
            </div>
        ...
        <div class="col-span-3 py-12 text-center">
            ...
            <p class="text-sm" style="color: #9CA3AF;">No hay demandados registrados</p>
        </div>
        @endforelse

        {{-- Mensaje si ningún demandado coincide con la búsqueda --}}
        <div x-show="[...document.querySelectorAll('.demandado-card')].every(el => el.style.display === 'none')" 
             class="col-span-3 py-12 text-center" 
             style="display: none;">
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#D1D5DB" stroke-width="1.5" class="mx-auto mb-3" aria-hidden="true"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            <p class="text-sm" style="color: #9CA3AF;">No se encontraron demandados que coincidan con la búsqueda o el filtro.</p>
        </div>
    </div>
